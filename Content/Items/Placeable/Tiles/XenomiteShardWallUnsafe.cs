...
{
    public class XenomiteShardWallUnsafe : ModItem
    {
        ...
        {
            DisplayName.SetDefault("Xenomite Shard Wall (Unsafe)");
            Tooltip.SetDefault("Holding this may infect you.");
            CreativeItemSacrificesCatalog.Instance.SacrificeCountNeededByItemId[Type] = 400;
        }
        public override void SetDefaults()
        {
            Item.DefaultToPlacableWall((ushort)ModContent.WallType<XenomiteShardWallTileUnsafe>());
            Item.width = 16;
            Item.height = 16;
            Item.maxStack = 999;
        }
            ...
            if (ModLoader.TryGetMod("Redemption", out var otherMod))
            {
                if (otherMod.TryFind<ModItem>("XenomiteShard", out var modItemName))
                {
                    CreateRecipe(4)
                        ...
                        .Register();
                    CreateRecipe(1)
                        .AddIngredient(ModContent.ItemType<XenomiteShardWall>())
                        .AddTile(TileID.WorkBenches)
                        .Register();
                }
            }
