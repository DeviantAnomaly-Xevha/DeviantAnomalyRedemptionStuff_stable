    ...
    public class XenomiteCrystalBombCharged_Proj : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            if (target.defDefense >= Projectile.damage * .75 || target.takenDamageMultiplier <= .25)
            {
                ...
                int a = Projectile.NewProjectile(Projectile.InheritSource(Projectile), new Vector2(Projectile.Center.X - 2, Projectile.Center.Y), new Vector2(-1f, -1f), Mod.Find<ModProjectile>("XenomiteCrystalBombChargedLeftHalf_Proj").Type, Projectile.damage / 6, 0, Main.myPlayer);
                Main.projectile[a].CritChance = Projectile.CritChance;
                int b = Projectile.NewProjectile(Projectile.InheritSource(Projectile), new Vector2(Projectile.Center.X + 2, Projectile.Center.Y), new Vector2(1f, -1f), Mod.Find<ModProjectile>("XenomiteCrystalBombChargedRightHalf_Proj").Type, Projectile.damage / 6, 0, Main.myPlayer);
